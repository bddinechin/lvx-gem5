...
| `shim.{cc,hh}`, `int256.{cc,hh}` | B | runtime API (`operandRead`/`operandFromValue`/`operandFromRegFile`/`commitRegFiles`, `MEM_*`, PC/branch, `syscall`) over `ExecContext`; FP → gem5 SoftFloat; int256_t is LVX-owned (header-only over __int128) |
...
| `process.cc`, `reg_abi.{cc,hh}` | B | SE process, LVX ABI arg/ret conventions, `scall` → `SyscallDesc` |
...
- Generated code uses `this` as a parameter name → compile the `generated/`
  unit as **C**, not C++.
- LVX binaries currently carry `e_machine = 256` (the machine number the LVX
  toolchain emits, pending a dedicated `EM_LVX`); gem5 recognizes

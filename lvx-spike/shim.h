...
#include <stdint.h>
/* Single source of truth for int256_t is the real gem5 arch shim; the spike used to
...
typedef struct Cpu {
    uint64_t gpr[64];
    struct { int gpr; int256_t val; } wb[32]; /* bundle write buffer */
    int nwb;
} Cpu;
...
typedef struct Insn {
    ...
    int256_t value[8];    /* operand slot values (populated by insn_fetch) */
} Insn;
